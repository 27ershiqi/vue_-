<template>
  <div class="pagination">
    <!-- 当前页码等于1就不可操作 -->
    <button :disabled="myCurrentPage===1" @click="changeCurrentPage(myCurrentPage-1)">上一页</button>
    <!-- 只有start大于1 -->
    ...
    <button v-for="item in startEnd.end" v-if="item>=startEnd.start" :key="item"
      @click="changeCurrentPage(item)" :class="{active: myCurrentPage===item}">
      {{item}}
    </button>
    ...
    <!-- 当前页码等于总页码就不可操作 -->
    <button :disabled="myCurrentPage===totalPages" @click="changeCurrentPage(myCurrentPage+1)">下一页</button>
    <!-- 总记录数 -->
